...
class UserModel:
    ...
    def connect(self):
        try:
            self.__connection = pymysql.connect(host='localhost', user='root', password='Hira*1614', database='webproject')
...
class ProductModel:
    ...
    def getProductByID(self,prodID):
        ...
        try:
            if self.__connection is None:
                self.connect()
            query = "select image,name,color,quantity,price from products where p_id = %s"
            args = (prodID)
            self.__cursor.execute(query,args)
            data = self.__cursor.fetchall()
            for row in data:
